...
Upon clicking 'View PDF', Node.js reroutes the user to the `GET /download/{file_name}` endpoint, and from there can view and download the thesis (through the user's PC operating system). 
...
### High-level Functionality
The system enables the user to ask questions regarding a specific thesis. The user enters a question about the thesis and clicks 'Ask Question'. FastAPI then sends a `GET /summarise/{db_id}` request, calls the `summarise_thesis` function in `gemini_ai_summariser.py`, where the following prompt is entered with the thesis in question referenced from the SQLite database: 
...
| Python                    | Summarises thesis with Gemini import |
