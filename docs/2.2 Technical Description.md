...
<ins>1. Separation of Concerns</ins>  
...
<ins>2. Efficiency of Information Retrieval</ins>  
...
<ins>4. Modularity and Extensibility</ins>  
...
<ins>5. Simplicity of User Interaction</ins>  
...
**System Functionality 1: Search Page Displayed**  

<ins>High-level Functionality</ins>  
...
<ins> Behavioural Requirements </ins>  
- BR1.1 – Accessible via web browser
- BR1.2 – Accessible via API
- BR2.1 – Entering search queries
- BR6.1 – Filtering by time period

<ins>Architecture & Component Interaction</ins>  
There are two components to display the search page:
| Component| Role|
|----------------------------------|--------------------------------------------------------------|
| Frontend (HTML/JS)               | HTML page, which is used to (1) render the user interface and (2) structure & style it. |
| Node.js Express server  | Serves static HTML, CSS, and JS.     |
...
**System Functionality 2: Search Query Processing**
...
<ins> Behavioural Requirements </ins>  
- BR2.1 – Entering search queries
- BR2.2 – Search returns relevant theses
- BR6.1 – Time period filtering
- BR7.2 – Background info returned (partially met)
- BR6.2 – OCR processing (partially met)
...
<ins> Search Processing Pipeline</ins>  
...
The query's vector embedding is compared against each thesis' precomputed title and abstract vector embedding in the database using cosine similarity. Each thesis' embedding is stored in `durham_thesis.index` and each embedding is mapped back to the corresponding row in the database using `durham_thesis_ids.npy` (both found in the `python` directory). This enables the system to obtain the theses with the closest semantic similarity to the user's search query.
...
**System Functionality 3: Thesis Access / Viewing**

<ins>High-level Functionality</ins>  
##### Accessing a Thesis
The system enables the user to view the PDF directly when the user explores the thesis further from initial results page. The user accesses the thesis PDF by clicking on the 'View PDF' link which is obtained from the thesis database after retrieving the search results (inside the `db` folder in the `python` directory). The following image demonstrates this feature, seen immediately under 'Full Thesis:' as a hyperlink ('View PDF'):

![Thesis Access Screenshot](diagrams/2.2/thesis_access.png)

##### Downloading a Thesis

Upon clicking 'View PDF', the user is rerouted (via hyperlink) to the thesis page on the Durham E-Theses website, and from there can view and download the thesis (through the user's PC operating system). 

<!-- Should this go in section 3? -->
<!-- For example, on macOS, when viewing the thesis in the separate tab, the thesis can be downloaded by navigating to 'File' in the menu bar and selecting 'Export as PDF'. This is visualised in the screenshot below:
...
**System Functionality 4: AI Question Answering for a Thesis**  

<ins>High-level Functionality</ins>  
The system enables the user to ask questions regarding a specific thesis. The user enters a question about the thesis and clicks 'Ask Question'. The frontend then sends a `GET /summarise/{db_id}` request to FastAPI, which calls the `summarise_thesis` function in `gemini_ai_summariser.py`, where the following prompt is entered with the thesis in question referenced from the SQLite database: 

*"Answer the following question using information provided in the thesis text below: "{query}". Be detailed in your explanation and include page numbers of information used to generate the response. Produce your summary in a HTML format, using only h5 and h6 for headings and subheadings."*


 The following image demonstrates this feature and the system's response when the user asks 'what is meant by liberal education?':

![AI Question SS](diagrams/2.2/ai_question.png)

*Note: The system does **not** remember responses to previous questions.*

<ins> Behavioural Requirements </ins>  
- BR4.1 – AI explanation of thesis
- BR4.2 – Page numbers returned

<ins>Architecture & Component Interaction</ins>  
There are two components involved when the user asks a question about a thesis:

| Component                                  | Role                                       |
|--------------------------------------------|--------------------------------------------|
| Frontend (HTML/JS)                         | HTML page, which is used to render the page including the 'Ask Question' button and display the system's response to the user.|
| FastAPI                                    | Routes user's thesis question to Gemini and sends Gemini's response back to the user. |

**System Functionality 5: AI Summarisation**

<ins>High-level Functionality</ins>  
The system enables the user to generate a detailed AI summary of the thesis in question. Upon clicking "Summarise Thesis", the frontend sends a JSON request to the FastAPI endpoint `GET /summarise/{db_id}` (Yes, the same one as as System Functionality 4), which is delegated to a dedicated summariser module. This summariser module loads the page-labelled thesis text, prompts Gemini, post-processes output to ensure format consistency (no html/code tags), and returns HTML for display. The following diagram shows the sequence of the AI Summarisation system functionality:

![AI Summarisation Diagram](diagrams/2.2/ai_summary_diagram.png)

<ins> Behavioural Requirements </ins>  
- BR5.1 – Generate summaries (partial)
- BR5.2 – Summaries include page numbers

<ins>Architecture & Component Interaction</ins>  
There are three components involved in summarising a theses:

| Component                 | Role                      |
|---------------------------|---------------------------|
| Frontend (HTML/CSS/JS)    | Handles web interface delivery | 
| FastAPI                   | API for summarise endpoints | 
| Summariser module (Python)| Summarises thesis with Gemini API (external) |
| SQLite Database           | Stores thesis to be summarised | 


#### 2.2.6 The Role of Behavioural-Driven Development in the Implementation Phase

The Behavioural-Driven Development (BDD) methodology was a significant contribution to keeping the technicalities of the implementation phase focused on fulfilling the project's objectives abstracted from client expectations. This includes: 

<ins>1. Feature Isolation (vertical slices)</ins>  
BDD supported parallel development of different architectural areas of the project. These eareas include:
- Web UI delivery (served by Node.js)
- Semantic search pipeline (FastAPI /search returning ranked results by relevance)
- Per-thesis AI analysis (FastAPI /summarise/{db_id} for single-thesis summarisation and thesis-specific question answering)

<ins>2. Iterative Validation</ins>  
Each sprint cycle derived scenarios from the list of behavioural requirements, and was used as a checklist for validation (e.g., “search returns relevant theses in ranked order”, “filters constrain results”, “AI responses include page numbers where possible”). This gave us early feedback and guided refinements to API responses and UI behaviour. For scenarios which couldn't be fulfilled, they were marked as met/not met with justification.
Overall, BDD maintained traceability between behavioural requirements, implemented system functionalities (see Section 2.2.5), and the final BR status recorded in Section 1.3.
